...
{
    /// <summary>
    /// Concrete graph view for the StarterGraph verification package.
    /// Hosts the canvas that displays <see cref="StarterGraph"/> assets.
    /// </summary>
    public class StarterGraphView : BaseGraphView
    {
        protected override BaseNodeView CreateNodeView(BaseNodeData node)
        {
            return node.NodeType switch
            {
                ...
                StarterStatementNodeData.NodeTypeId => new StarterStatementNodeView((StarterStatementNodeData)node),
        ...
        public override void BuildContextualMenu(UnityEngine.UIElements.ContextualMenuPopulateEvent evt)
        {
            ...
            {
                var node = new StarterStatementNodeData { NodeType = StarterStatementNodeData.NodeTypeId };
                AddNodeToCanvas(node, mousePos);
            });
