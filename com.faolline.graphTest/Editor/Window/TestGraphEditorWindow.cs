...
{
    /// <summary>
    /// Editor window for the GraphTest verification package.
    /// Opens via <c>Faolline/Open TestGraph Editor</c> or by double-clicking a <see cref="TestGraph"/> asset.
    /// Maintains a persistent runner session enabling GoBack and GoBackToCheckpoint after a Run.
    /// </summary>
    public class TestGraphEditorWindow : BaseGraphEditorWindow
    {
        // ── Session state ─────────────────────────────────────────────────────
        ...
        [MenuItem("Faolline/Open TestGraph Editor")]
        public static void Open()
        {
            GetWindow<TestGraphEditorWindow>("TestGraph Editor");
        }
        ...
        {
            _testInspector = new TestNodeInspectorView();
            return _testInspector;
        }

        private TestNodeInspectorView _testInspector;

        protected override void OnGraphLoaded(BaseGraph graph)
        {
            _testInspector?.SetGraph(graph);
            _testInspector?.SetGraphView(GraphView as TestGraphView);
        }
        ...
        private void ShowChooseMenu()
        {
            ...
            {
                string label = (choice is TestChoice tc && !string.IsNullOrEmpty(tc.Label)) ? tc.Label : choice.Id;
                string id = choice.Id;
        ...
        public void ExecuteGraph(BaseGraph graph)
        {
            ...
            _activeContext = new TestGameContext();
