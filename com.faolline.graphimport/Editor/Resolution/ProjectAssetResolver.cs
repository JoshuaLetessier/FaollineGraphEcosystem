...
namespace Faolline.GraphImport.Editor
{
    ...
    /// project or guesses a path. This means it resolves cleanly for a sub-dialogue link whose
    /// target is being generated in the SAME run; a target that was generated in an earlier run,
    /// or hand-authored outside this pipeline, still resolves to null (the documented-safe
    /// "incomplete node" state), consistent with the rest of the pipeline's "never guess" precedent.
    ///
