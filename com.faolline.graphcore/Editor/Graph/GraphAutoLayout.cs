...
namespace Faolline.GraphCore.Editor
{
    ...
    {
        /// <summary>Horizontal gap between layers (columns).</summary>
            ...
            float columnSpacing = ColumnSpacing, float rowSpacing = RowSpacing)
        {
            ...
            for (int L = 0; L < layers.Count; L++)
            {
                ...
                for (int r = 0; r < lst.Count; r++)
                    result[lst[r]] = new Vector2(L * columnSpacing, (r - offset) * rowSpacing);
            }
            return result;
        }
        ...
        /// (or same) columns get none (a straight orthogonal link is already clean). Pair with <see cref="Arrange"/>.
        /// </summary>
        public static Dictionary<string, List<Vector2>> RouteLongEdges(
            ...
            float columnSpacing = ColumnSpacing, float nodeHeight = 90f)
        {
            var result = new Dictionary<string, List<Vector2>>();
            if (nodePositions == null || nodePositions.Count == 0 || edges == null) return result;

            float maxY = float.MinValue;
            foreach (var kv in nodePositions) if (kv.Value.y > maxY) maxY = kv.Value.y;
            float laneBase = maxY + nodeHeight + 70f;
            ...
            foreach (var e in edges)
            {
                ...
                int span = Mathf.RoundToInt(Mathf.Abs(to.x - from.x) / columnSpacing);
                if (span < 2) continue;   // adjacent / same column → a straight orthogonal link is already clean
                ...
                float x1 = from.x + columnSpacing * 0.85f;
                float x2 = to.x - columnSpacing * 0.15f;
