...
{
    /// <summary>
    /// Universal condition: reads a <see cref="VariableDef"/> (int) from the context and compares it to an
    ...
    public class IntCondition : BaseCondition, IVariableReferencing
    {
        [SerializeField, Tooltip("Variable asset to read and compare. Drag a VariableDef (type Int).")]
        ...
        public override bool Evaluate(BaseContext context)
        {
            ...
            try
            {
                if (!context.TryGet<int>(_variable, out value))
                {
                    if (_warnOnMissing)
                        Logging.Warning("GraphCore.Runtime", $"[GraphCore] IntCondition: parameter '{_variable.DisplayName}' not found — false.");
                    return false;
                }
            }
            catch (System.InvalidCastException)
            {
                Logging.Warning("GraphCore.Runtime", $"[GraphCore] IntCondition: parameter '{_variable.DisplayName}' is not an int — false.");
                return false;
            }
